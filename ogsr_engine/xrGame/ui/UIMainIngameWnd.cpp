...
void CUIMainIngameWnd::SetAmmoIcon(const shared_str& sect_name)
{
    ...
    float w = std::clamp(iGridWidth, 1.f, 2.f) * INV_GRID_WIDTH / 2;
    float h = INV_GRID_HEIGHT / 2;
    w *= UI()->get_current_kx();
...
void CUIMainIngameWnd::ShowBooster(EBoostersIcons icon) { booster_icon_list[icon]->Show(true); }

void CUIMainIngameWnd::HideBooster(EBoostersIcons icon) { booster_icon_list[icon]->Show(false); }
