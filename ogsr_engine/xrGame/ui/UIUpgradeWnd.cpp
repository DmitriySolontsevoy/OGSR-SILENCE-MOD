...
{
	GetMessageTarget()->SendMessage(this, UPGRADE_WND_CLOSED);
}

void CUIUpgradeWnd::StartUpgrade()
{
	bStarted = true;
}

void CUIUpgradeWnd::StopUpgrade()
{
	bStarted = false;
}
...
bool CUIUpgradeWnd::CanMoveToOther(PIItem pItem, bool our)
{
	return true;
}
...
{
	m_uidata->UIStaticMain.Enable(false);
}

void CUIUpgradeWnd::EnableAll()
{
	m_uidata->UIStaticMain.Enable(true);
}
...
bool CUIUpgradeWnd::ToRepairBox(CUICellItem* itm, bool upgraded)
{
	...
	if (!upgrader.IsUpgradable(item)) return false;
	...
	if (m_uidata->UIRepairBox.ItemsCount() > 0)
	{
		...
		if (!upgraded) move_item_(m_uidata->UIRepairBox.GetItemIdx(0), &m_uidata->UIRepairBox, &m_uidata->UIOurBagList);
	}

	if (!upgraded) move_item_(itm, &m_uidata->UIOurBagList, &m_uidata->UIRepairBox);
	...
	{
		if (counter == 10 || counter == upgrades.size()) break;
