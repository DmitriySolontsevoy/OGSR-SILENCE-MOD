...
void CUIDragDropListEx::SetHighlightCellSp(bool b) { m_flags.set(flHighlightCellSp, b); }

bool CUIDragDropListEx::GetHighlightCellSp() { return !!m_flags.test(flHighlightCellSp); }

void CUIDragDropListEx::SetHighlightAllCells(bool b) { m_flags.set(flHighlightAllCells, b); }
...
Ivector2 CUICellContainer::TopVisibleCell() { return Ivector2().set(0, iFloor(m_pParentDragDropList->ScrollPos() / float(CellSize().y + CellsSpacing().y))); }
    ...
    if (m_pParentDragDropList->GetHighlightAllCells() && m_pParentDragDropList->ItemsCount())
    {
        for (int x = 0; x <= tgt_cells.width(); ++x)
        {
            for (int y = 0; y <= tgt_cells.height(); ++y)
            {
                ...
                if (parent_select_mode != CellTextureTypeNormal)
                    break;
            }
            if (parent_select_mode != CellTextureTypeNormal)
                break;
