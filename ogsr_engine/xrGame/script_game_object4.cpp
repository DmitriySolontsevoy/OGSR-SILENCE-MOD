...
bool CScriptGameObject::is_body_turning() const
{
    ...
    if (!stalker)
        ...
        return (!fsimilar(stalker->movement().head_orientation().target.yaw, stalker->movement().head_orientation().current.yaw) ||
                !fsimilar(monster->movement().body_orientation().target.yaw, monster->movement().body_orientation().current.yaw));
}
...
Fmatrix* CScriptGameObject::GetXForm() { return &(object().renderable.xform); }
